...
import { ProfessionalInfo, ProfessionalInfoService, ProfessionalQualification, ProfessionalQualificationService, ProfessionalRegistration, ProfessionalRegistrationService, cleanObject } from '../../../../../core';
...
export class RegistrationService {
  private _data$ = new BehaviorSubject<ProfessionalRegistration[]>([]);
  ...
  constructor(private _baseService: ProfessionalRegistrationService) { }
...
public getBaseData() {
  ...
  get registrationName() {
    return this._option.registrationName;
  }
  set registrationName(registrationName: string) {
    registrationName ? (this._option.registrationName = registrationName) : this._set({ registrationName });
  }

  get registrationDate() {
    return this._option.registrationDate;
  }
  set registrationDate(registrationDate: string) {
    registrationDate ? (this._option.registrationDate = registrationDate) : this._set({ registrationDate });
  }

  get certificatePhoto() {
    return this._option.certificatePhoto;
  }
  set certificatePhoto(certificatePhoto: string) {
    certificatePhoto ? (this._option.certificatePhoto = certificatePhoto) : this._set({ certificatePhoto });
  }
